...
class TicTacToe:
    ...
    def take_manual_turn(self, player):
        ...
        while not valid:
            try:
                ...
                print("Please enter a valid move.")
    def minimax_pruning(self, player, alpha=-1000, beta=1000, depth=10):
        #X is negative numbers, O is positive numbers
        if self.check_win("X"):
            return (-1, 0, 0)
        ...
        if player == "O":
            ...
            for r in range(self.NUM_ROWS):
                for c in range(self.NUM_COLS):
                    ...
                    if self.board[r][c] == "-":
                        ...
                        new_score = self.minimax_pruning("X", alpha, beta, depth-1)[0]
                        if new_score > alpha:
                            alpha = new_score
                            if alpha >= beta:
                                no_consider = True
                        if new_score > result[0]:
                            result = (new_score, r, c)
            ...
            for r in range(self.NUM_ROWS):
                for c in range(self.NUM_COLS):
                    if self.board[r][c] == "-":
                        self.place_player(player, r, c)
                        new_score = self.minimax_pruning("O", alpha, beta, depth-1)[0]
                        if new_score < beta:
                            beta = new_score
                            if alpha >= beta:
                                no_consider = True
        ...
        return result
    def minimax(self, player, depth=10):
        #X is negative numbers, O is positive numbers
        if self.check_win("X"):
            return (-1, 0, 0)
        elif self.check_win("O"):
        ...
        if player == "O":
            ...
            for r in range(self.NUM_ROWS):
                for c in range(self.NUM_COLS):
                    if self.board[r][c] == "-":
                        self.place_player(player, r, c)
                        new_score = self.minimax("X", depth-1)[0]
                        if new_score > result[0]:
                            result = (new_score, r, c)
                        self.place_player("-", r, c)
        else:
            ...
            for r in range(self.NUM_ROWS):
                for c in range(self.NUM_COLS):
                    if self.board[r][c] == "-":
                        self.place_player(player, r, c)
                        new_score = self.minimax("O", depth-1)[0]
                        if new_score < result[0]:
                            result = (new_score, r, c)
                        self.place_player("-", r, c)
    ...
    def take_computer_turn(self, player):
        ...
        result = self.minimax_pruning(player, False)
        print(result)
        self.place_player(player, result[1], result[2])
    ...
    def take_random_turn(self, player):
        ...
        while True:
            r,c = random.randint(0, self.NUM_ROWS-1), random.randint(0, self.NUM_COLS-1)
    ...
    def take_turn(self, player):
        ...
        if player == self.player1:
            ...
            self.take_computer_turn(player)
